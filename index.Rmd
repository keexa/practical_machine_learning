...
It is possible to see that there are many columns which are providing values related to accelerometers.

The columns of the sets have been reduced considering that the original testing set (pml-testing.csv) has many columns with values not provided. 

It is clear that the columns which are a calculation on the other columns (average, max, min, etc) have to be removed as they represent only specific rows of the training set (the ones at the end of 150ms window).
...
# Discarding the not meaningful columns
...
Only one round of cross-validation is performed for the sake of simplicity in this project, but it would be nice to have multiple round on several partitionings and then average them.
...
We then remove from the training and test data the features which have high correlation
```{r, echo=TRUE}
...
print(model)
```
We can then see that the **estimated out of sample error** is around **0.5%**.
